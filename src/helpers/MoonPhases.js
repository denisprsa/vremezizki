...
function moonCoords(d) { 
    ...
    let distanceToMoonKM = 385001 - 20905 * cos(M);
...
function getMoonPosition(date, lat, lng) {
    ...
    let pa = atan(sin(H), tan(phi) * cos(c.dec) - sin(c.dec) * cos(H));
...
export function PhaseHunt(date) {
    if (!date) {
        date = new Date()
    }
  
    let aDate = new Date(date.getTime() - (45 * 86400000)) // 45 days prior
    let k1 = Math.floor(12.3685 * (aDate.getFullYear() + (1.0 / 12.0) * aDate.getMonth() - 1900))
    let nt1 = meanPhase(aDate.getTime(), k1)
  
    let julianDate = toJulian(date)
...
export function MoonIllumination(date) {
    ...
    let distanceEarthSunKM = 149598000;

    let phi = acos(sin(s.dec) * sin(m.dec) + cos(s.dec) * cos(m.dec) * cos(s.ra - m.ra));
    let inc = atan(distanceEarthSunKM * sin(phi), m.dist - distanceEarthSunKM * cos(phi));
...
export function MoonTimes(date, lat, lng, inUTC) {
    let t = new Date(date);
